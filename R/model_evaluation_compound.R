...
load("/data/Untitled/Trabajo/R_practice/exp_compound/data/trainindices/ERA5_monthly_1940_2022_indTRAIN1.rda")
samplesize <- nrow(data)
learndata <- data[indTRAIN1,]
indTEST1 <- (1:samplesize)[-indTRAIN1]
testdata <- data[indTEST1,]
############################################################################
#
############################################################################
# 
...
loadIterationsComp <- function(permused = 0, algo = c("hc","tabu"),score = c("aic","bic"), it = NULL,train = NULL) {
  
  if(!is.null(train)){hc_list <- list.files(paste0("data/",algo,"iterations/ERA5_monthly_1940_2022_",degrees,"_disc_",b,"_",score,lim,comp,combi,superf,n1,"/perm",k,"train"), full.names = T)
  hc_names <- list.files(paste0("data/",algo,"iterations/ERA5_monthly_1940_2022_",degrees,"_disc_",b,"_",score,lim,comp,combi,superf,n1,"/perm",k,"train"))
  hc_names <- gsub(".rda", "", hc_names)} else {
...
###############################################################
tabu_bic_0 <- loadIterationsComp(permused = k,algo = algo, score = score)
tabu_bic_0_fits<- lapply(tabu_bic_0, function(x) bn.fit(x = x, data = data))
tabu_bic_0_train <- loadIterationsComp(permused = k,algo = algo, score = score, train = TRUE)
tabu_bic_0_train_fits<- lapply(tabu_bic_0_train, function(x) bn.fit(x = x, data = learndata))

lo_tabu_bic_0<- sapply(X = tabu_bic_0_fits,FUN =logLik,data = data)
lo_tabu_bic_0_train <- sapply(X = tabu_bic_0_train_fits,FUN =logLik,data = learndata)
lo_tabu_bic_0_test <- sapply(X = tabu_bic_0_train_fits,FUN =logLik,data = testdata)
which(lo_tabu_bic_0_test == max(lo_tabu_bic_0_test))

stb0 <-sapply(FUN = narcs, tabu_bic_0)
stb0_train <-sapply(FUN = narcs, tabu_bic_0_train)

plotname <- paste0("figs/loglik_",algo,"_",score,"_",degrees,"_disc_",b,lim,comp,combi,superf,n1,"_train_test")
pdf(paste0(plotname,".pdf"))
png(paste0(plotname,".png"))

plot(stb0_train,lo_tabu_bic_0_train,col = rainbow(2)[1],pch = 16, xlab = "|E|",ylab = c("Loglik(x|model)"))
points(stb0_train,lo_tabu_bic_0_test,col = rainbow(2)[2],pch = 16)

legend("bottomright",legend = c("tabu train aic","tabu test aic"), fill = rainbow(2),cex = 0.7)
dev.off()
